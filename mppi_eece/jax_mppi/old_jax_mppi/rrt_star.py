...
class RRTStar:
    ...
    def find_path(self, start, end, **kwargs):
        self.start_node = Node(start)
        self.end_node = Node(end)
        self.node_list = [self.start_node]
        
        start_time = time.time()
        # Use a KD-tree for faster nearest/near queries when node count grows.
        kdtree = None
        rebuild_every = 5
        last_rebuild = 0
        for i in range(self.max_iter):
            ...
            # rebuild KD-tree periodically for current node positions
    ...
    def plot_rrt_progress(self, rnd_node=None):
        plt.clf()
        # Plot obstacles
        if self.collision_checker.occup_grid is not None:
            ox, oy = [], []
            occupied = self.collision_checker.get_all_nonzero()
            plt.plot(occupied[:,0], occupied[:,1], ".k", markersize=2)

        # Plot start and goal
        plt.plot(self.start_node.pos[0], self.start_node.pos[1], "gs", markersize=8, label='Start')
        plt.plot(self.end_node.pos[0], self.end_node.pos[1], "rs", markersize=8, label='Goal')

        # Plot nodes and connections
        for node in self.node_list:
            if node.parent:
                plt.plot([node.pos[0], node.parent.pos[0]],
                         [node.pos[1], node.parent.pos[1]], "-g")

        if rnd_node:
            plt.plot(rnd_node.pos[0], rnd_node.pos[1], "^c")
    ...
    def get_random_node(self):
        origin = self.collision_checker.origin
        wh = self.collision_checker.wh
        resolution = self.collision_checker.resolution
        if np.random.randint(0, 100) > self.goal_sample_rate * 100:
            rnd = Node([np.random.uniform(origin[0], origin[0] + wh[0] * resolution),
                        np.random.uniform(origin[1], origin[1] + wh[1] * resolution),
                        0.0])
    ...
    def find_near_nodes(self, new_node, kdtree=None):
        """Return list of indices of nodes within the connection radius r.

        If a KD-tree is provided we use a radius query which is faster for
        larger node counts. Otherwise fall back to linear scan.
        """
    ...
    def choose_parent(self, new_node, near_inds):
        if not near_inds:
            return None
        # Compute candidate costs first (cheap), then perform a single
        # vectorized collision check over all candidates using the
        # collision checker batch API. This reduces Python-level looping
        # and lets numpy do the heavy lifting.
        cost_pairs = []  # (cost, index)
        for i in near_inds:
        ...
        # Build arrays for batch collision check: starts = near_node.pos[:2], ends = new_node.pos[:2]
        starts = np.array([self.node_list[i].pos[:2] for _, i in cost_pairs])
        ends = np.tile(new_node.pos[:2], (len(cost_pairs), 1))

        # Use the vectorized line free checker. It returns True if the line is free.
        if len(starts) == 0:
            return None

        free_mask = self.collision_checker.is_line_free_batch(starts, ends, num_samples=10, early_exit=False)

        # find the first candidate (lowest cost) that is collision-free
    ...
    def rewire(self, new_node, near_inds):
        # Collect candidates where rewiring would improve cost
        candidates = []  # (index, new_cost)
        for i in near_inds:
            near_node = self.node_list[i]
        ...
        # Batch collision check from new_node to each candidate near_node
        starts = np.tile(new_node.pos[:2], (len(candidates), 1))
        ends = np.array([self.node_list[i].pos[:2] for i, _ in candidates])

        free_mask = self.collision_checker.is_line_free_batch(starts, ends, num_samples=10, early_exit=False)

        # Apply rewires for those that are free and improve cost
