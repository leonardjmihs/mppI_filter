...
"""
GSF-MPPI hybrid controller (clean, readable rewrite)

This module implements a Gaussian-Sum Filter style representation over
control sequences but uses *MPPI-style* importance weights to push
probability mass toward low-cost trajectories.

Design goals (Option A from conversation):
 - Keep UKF-style prediction for means/covariances of Gaussian components
 - Use MPPI exponential-of-cost weighting (log-space) for components
 - Keep mixture management: prune -> merge -> cap
 - Keep code readable, avoid mixing JAX/NumPy in confusing ways
 - Keep planner and dynamics as external objects passed via params

Primary entry point: do_gsf_mppi(params)

Key params (examples):
 - dt, Nt (horizon), start (initial state), q_ref (goal), nlmodel
 - sigma0 (scalar or matrix), process_noises (list of Q modes),
   measurement_noises (kept for compatibility but only mode weights used),
 - mppi_lambda (temperature), wmin, gamma (merge thresh), Jmax
 - planner (object with eval_U_seq(u_seq, theta, x, q_ref, cost_map) -> [cost,...])

Notes:
 - This implementation evaluates cost at the component mean only (fast).
   If you want to average over sigma points, the place to do that is clearly
   indicated in the code.
 - No fake "y_meas = 0" used.
 - No JIT/JAX internals used here. If you have a jax-based planner/dynamics
   you may still pass them, but mixing jax arrays with numpy is not done.
