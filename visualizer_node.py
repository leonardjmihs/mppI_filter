...
class VisNode(Node):
    ...
    def safe_zones_cb(self, msg: MarkerArray):
        ...
        for marker in msg.markers:
            ...
            self.safe_zones.append((cx, cy, radius))
            ...
            #         self.safe_zones.append([pt.x, pt.y])
            # else:
    ...
    def vis_all_cont_cb(self, msg: Float32MultiArray):
        ...
        try:
            if N_mini > 0:
                ...
                zones = np.array(self.safe_zones)   
                self.log.info(f"[vis_all_cont] #safe_zones = {zones.shape[0]}")

                for i in order[:m_elite]:
                    sample = contingencies[i]    # shape (N_safe, N_mini, state_dim)
                    good = True

                    for j, (cx, cy, radius) in enumerate(self.safe_zones):
                        pts   = sample[j, :, :2]               
                        dists = np.linalg.norm(pts - [cx, cy], axis=1)
                        reached = np.any(dists <= radius + 100.5)   

                        self.log.info(
                            f"[vis_all_cont] candidate {i}, zone {j}, "
                            f"min_dist={dists.min():.2f}, radius={radius:.2f}, reached={reached}"
                        )
                    ...
                    if good:
                        mini_guys.append(sample.reshape(-1, sample.shape[-1]))
                        main_state_seq.append(sample[:, 0, :])
                        break



                # self.log.info(f"[vis_all_cont] #candidates after filter = {len(mini_guys)}")
