...
class GSF_Controller_Expanded:
    ...
    def measurement_function(self, theta, x_current, q_ref, cost_map):
        ...
        # theta shaped to U sequence
        u_seq = theta.reshape((-1, self.n_u))
        # planner.eval_U_seq returns (cost, terminal_state, state_seq, ...) in your version
        out = self.planner.eval_U_seq(u_seq, theta, x_current, q_ref, cost_map)
        cost = out[0]
        # clamp cost to reasonable values
